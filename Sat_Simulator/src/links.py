...
import itur
import numpy as np
from src.const import SNRMechanism
...
class Link:
    ...
    def create_link(satellites: 'Union[List[Satellite], Satellite]', stations: 'Union[List[Station], Station]', time: Time) -> 'List[Link]':
        ...
        ##Create link objects
        outLinks = [Link(sat, ground, time, downlinkDatarate=3900, snr=snr, distance=dist) for sat, ground, snr, dist in zip(satellites, stations, snrs, distance)]
    ...
    def __init__(self, sat: Satellite, gs: Station, time: Time, **kwargs) -> None:
        ...
        # print(self.downlinkDatarate)
        # print(self.uplinkDatarate)
    ...
    def per_from_ber(self, ber):
        ...
        return binom.sf(const.ALLOWED_BITS_WRONG, packet_size, ber)
