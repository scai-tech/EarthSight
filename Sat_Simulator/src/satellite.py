...
class Satellite (Node):
    ...
    def setup_skyfield(self):
        ...
        if const.INCLUDE_POWER_CALCULATIONS and Satellite.SunPos is None and Satellite.EarthPos is None:
            ...
            data = load('referenceData/de440s.bsp')
    ...
    def update_orbit(self, time: Time) -> Location:
        self.position = self.calculate_orbit(time)
    ...
    def calculate_orbit(self, time: Time) -> Location:
        ...
        dt = time.to_datetime()
        if dt in self.storedPositions:
            loc = self.storedPositions[dt]
        elif self.hasTle:
            loc = self.calculate_orbit_with_tle(time, self.tle)
            self.storedPositions[dt] = loc
        elif self.hasKepler:
        ...
        self._orbit_call_count += 1
        if self._orbit_call_count % 500 == 0:
            cutoff = dt - timedelta(hours=2)
            self.storedPositions = {k: v for k, v in self.storedPositions.items() if k >= cutoff}
    ...
    def calculate_orbit_at_multiple_times(self, startTime: Time, endTime: Time, timeStep: float) -> "Dict[datetime, Location]":
        ...
        self.storedPositions.update(out)
        return out
