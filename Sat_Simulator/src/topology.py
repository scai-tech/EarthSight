...
class Topology:
    ...
    def __init__(self, time: Time, satList: 'List[Satellite]', groundList: 'List[Station]') -> None:
        ...
        self.possibleDownLinks, self.nodeDownLinks = self.create_possible_links(self.availableMap, self.time)
        self.possibleUpLinks, self.nodeUpLinks = self.create_possible_links(self.availableMap, self.time)
        self.nFootprints = {sat: sum([1 for link in self.nodeDownLinks[sat] if link.gs.transmitAble]) for sat in self.satList}
    #@profile
